...
export class RuleSetService implements OnModuleInit {
  ...
  private sampleFiles = ['sample-netpay-calc', 'sample-utility-bill',  'sample-table-lookup', 'sample-good-life',  'sample-motor-insurance','sample-health-insurance','sample-loan-eligibility', 'sample-statistics', 'sample-survey', 'sample-math-js'];
  ...
  async persistBusinessLogic(setName: string) {
    ...
    if(this.sampleFiles.includes(setName)){
      throw new BadRequestException(`Overwriting sample RuleSets is not allowed. Create a new ruleset instead and copy individual rules from samples.`);
      }
